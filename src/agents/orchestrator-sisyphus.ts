...
delegate_task(category="general", load_skills=["playwright"], prompt="...")  // Browser testing
delegate_task(category="visual-engineering", load_skills=["frontend-ui-ux", "playwright"], prompt="...")  // UI with browser testing
\`\`\`
...
After \`delegate_task()\` completes, you MUST verify EVERY claim:
...
**DELEGATION TARGETS:**
- \`delegate_task(category="ultrabrain")\` → backend/logic implementation
- \`delegate_task(category="visual-engineering")\` → frontend/UI implementation
- \`delegate_task(subagent_type="git-master")\` → ALL git commits
- \`delegate_task(subagent_type="document-writer")\` → documentation
- \`delegate_task(subagent_type="debugging-master")\` → complex debugging

**⚠️ Default: run_in_background=false (sync execution).**
